...
make_EHelper(add) {
  ...
  // update CF
  rtl_is_add_carry(&s0,&s1, &id_src->val); //dest, res, src1
  rtl_is_add_carry(&s1, &s1, &id_dest->val);
  rtl_or(&s0, &s0, &s1);
  rtl_set_CF(&s0);

  // update OF
  rtl_is_add_overflow(&s0, &res, &id_dest->val, &id_src->val, id_dest->width);
  rtl_set_OF(&s0);
...
make_EHelper(cmp) {
  ...
  rtl_is_sub_carry(&s1,&s0, &id_dest->val); //dest, res, src1
...
make_EHelper(dec) {
  s0=1;
  rtl_sub(&id_dest->val,&id_dest->val,&s0);
  operand_write(id_dest, &id_dest->val);
  rtl_update_ZFSF(&id_dest->val, id_dest->width);
...
make_EHelper(sbb) {
  ...
  // update OF
  rtl_is_sub_overflow(&s0, &res, &id_dest->val, &id_src->val, id_dest->width);//dest, res, src1, src2, width
